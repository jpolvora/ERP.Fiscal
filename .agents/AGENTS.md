...
- **Product-only skills (this repo):** `security-check`, `sync-plugnotas-docs`, `code-review`, `erp-fiscal-consumer` (`consume-erp-fiscal`), and `release-nuget-package` are routed from root `AGENTS.md` only — not duplicated here. Bodies may be pt-BR; portable pipeline skills stay en-us per **Language** above.
