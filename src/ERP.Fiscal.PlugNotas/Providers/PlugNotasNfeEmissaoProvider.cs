...
/// <summary>Implementação PlugNotas de <see cref="INfeEmissaoProvider"/>. Consumo apenas via a interface (DI).</summary>
internal class PlugNotasNfeEmissaoProvider : INfeEmissaoProvider
{
    ...
    private readonly ILogger<PlugNotasNfeEmissaoProvider> _logger;

    public PlugNotasNfeEmissaoProvider(
        PlugNotasHttpClient http,
        IOptions<PlugNotasOptions> options,
        ILogger<PlugNotasNfeEmissaoProvider> logger)
    {
        _http = http;
        _options = options;
        _logger = logger;
    ...
    public async Task<NfeEmissaoResult> EmitirAsync(string payloadJson, NfeAmbiente ambiente, CancellationToken cancellationToken = default)
    {
        ...
        var raw = await _http.EmitirNfeAsync(baseUrl, apiToken, payloadJson, cancellationToken);
    ...
    {
        var (baseUrl, apiToken) = ResolveEndpoint(ambiente);
        var raw = await _http.ObterNfeResumoPorIdAsync(baseUrl, apiToken, idDocumentoProvedor, cancellationToken);
    ...
    {
        var (baseUrl, apiToken) = ResolveEndpoint(ambiente);
        var raw = await _http.ObterNfeResumoPorCnpjIdIntegracaoAsync(baseUrl, apiToken, cpfCnpjDigits, idIntegracao, cancellationToken);
        return MapConsultaResult(raw.Success, raw.HttpStatusCode, raw.RawBody, raw.ErrorMessage);
    }
    ...
    {
        var payloadEnviado = PlugNotasNfePayloadAmbienteHelper.AplicarProducaoNoPayloadJson(payloadJson, ambiente, _logger);
        ...
        if (consulta == null)
        {
            return new NfeProcessamentoResult
            {
                ...
                Mensagem = "Documento enviado; aguardando retorno SEFAZ (consulte o status).",
    ...
    {
        var (baseUrl, apiToken) = ResolveEndpoint(ambiente);
        var raw = await _http.CancelarNfeAsync(baseUrl, apiToken, idDocumentoProvedor, justificativa, cancellationToken);
    ...
    {
        var (baseUrl, apiToken) = ResolveEndpoint(ambiente);
        var raw = await _http.ObterXmlNfePorIdAsync(baseUrl, apiToken, idDocumentoProvedor, cancellationToken);
    ...
    {
        var (baseUrl, apiToken) = ResolveEndpoint(ambiente);
        var raw = await _http.ObterPdfNfePorIdAsync(baseUrl, apiToken, idDocumentoProvedor, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            Sucesso = false,
            Mensagem = "Identificadores insuficientes para consulta NF-e"
        };
    }
        ...
        string? idIntegracao)
    {
        return new NfeProcessamentoResult
        {
            ...
            Mensagem = emissao.Mensagem ?? "Falha na emissão PlugNotas",
            PayloadEnviado = payloadEnviado,
            RawResponse = emissao.RawResponse,
            IsTransientFailure = emissao.IsTransientFailure,
            HttpStatusCode = emissao.HttpStatusCode
